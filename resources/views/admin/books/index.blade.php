...
@section('content')
    <div class="p-3 p-md-4">
        <div class="d-flex align-items-center justify-content-between flex-wrap gap-3 mb-4">
            <div>
                <h1 class="fw-bold mb-0 h5" style="letter-spacing:-0.02em;">Books</h1>
            </div>
            <a href="{{ route('admin.books.create') }}" class="btn btn-primary fw-semibold px-3">
        ...
        <div class="card border rounded-4 overflow-hidden shadow-sm">
            <div class="dt-top-bar">
                <span class="fw-semibold small">All Books</span>
                <div class="d-flex align-items-center gap-2 flex-wrap" id="dt-controls-top"></div>
            </div>
            <div class="table-responsive">
                <table id="bookTable" class="table table-hover align-middle mb-0 w-100">
                    ...
                    <tbody>
                        @foreach ($books as $key => $book)
                            <tr>
                                ...
                                <td>
                                    @if($book->image)
                                        <a href="{{ asset('assets/books/' . $book->image) }}" target="_blank">
                                            <img src="{{ asset('assets/books/' . $book->image) }}"
                                                 alt="{{ $book->title }}"
                                                 style="width:48px;height:48px;object-fit:cover;border-radius:8px;border:1px solid #dee2e6;">
                                        </a>
                                    @else
                                        <span class="text-secondary"><i class="bi bi-image" style="font-size:1.4rem;"></i></span>
                                    @endif
                                </td>
                                <td>{{ $book->quantity }}</td>
                                <td>
                                    <div class="d-flex gap-1">
                                        ...
                                        <a href="{{ route('admin.books.edit', $book) }}" class="btn-action" title="Edit">
                                            <i class="bi bi-pencil"></i>
                                        </a>
                                        <a href="{{ route('admin.books.destroy', $book) }}" class="btn-action"
                                            title="Delete Book"
                                            onclick="return confirm('This will permanently delete the book. Continue?')">
    ...
    <script>
        $(document).ready(function() {
            ...
            function updateBottomBar() {
                ...
                $('#dt-info').text(
                    `Showing ${start}–${info.end} of ${info.recordsDisplay} books`
                );
